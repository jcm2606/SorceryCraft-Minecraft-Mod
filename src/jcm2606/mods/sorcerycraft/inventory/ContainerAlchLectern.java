...
public class ContainerAlchLectern extends Container {
	...
	public ContainerAlchLectern(InventoryPlayer par1InventoryPlayer,
			World par2World, int par3, int par4, int par5) {
		worldObj = par2World;
		posX = par3;
		posY = par4;
		posZ = par5;
		addSlotToContainer(new SlotCrafting(par1InventoryPlayer.player,
		...
		for (var6 = 0; var6 < 3; ++var6) {
			for (var7 = 0; var7 < 9; ++var7) {
				addSlotToContainer(new Slot(par1InventoryPlayer, var7 + var6
						* 9 + 9, 8 + var7 * 18, 84 + var6 * 18));
			}
		}

		for (var6 = 0; var6 < 9; ++var6) {
			addSlotToContainer(new Slot(par1InventoryPlayer, var6,
					8 + var6 * 18, 142));
		}
	...
	public void onCraftMatrixChanged(IInventory par1IInventory) {
        ...
        {
            this.craftResult.setInventorySlotContents(0, CraftingManagerAlchLectern.getInstance().findMatchingRecipe(this.craftMatrix, this.worldObj));
        }
	}
	...
	public boolean canInteractWith(EntityPlayer par1EntityPlayer) {
		return worldObj.getBlockId(posX, posY, posZ) != SCObjects.alchpodium.blockID ? false
				: par1EntityPlayer.getDistanceSq(posX + 0.5D, posY + 0.5D,
						posZ + 0.5D) <= 64.0D;
	}
