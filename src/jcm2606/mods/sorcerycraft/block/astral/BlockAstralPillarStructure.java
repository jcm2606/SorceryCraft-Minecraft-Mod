...
public class BlockAstralPillarStructure extends SCBlock implements IBlockRenderHandler
{
    public Icon icon;
    
    public BlockAstralPillarStructure(int par1)
    {
        super(par1, Material.rock, "astralPillarStructure", Rarities.BASIC);
    ...
    public boolean renderWorld(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer)
    {
        ...
        if(this.checkValidity(world, x, y, z))
        {
            t.setBrightness(255);
        }
    ...
    private boolean checkValidity(IBlockAccess world, int x, int y, int z)
    {
        boolean success = false;
        
        TileEntity te = null;
        
        if(world.getBlockId(x, y - 1, z) == SCObjects.astralPillarBase.blockID)
        {
            te = world.getBlockTileEntity(x, y - 1, z);
        } else
            if(world.getBlockId(x, y - 2, z) == SCObjects.astralPillarBase.blockID)
            {
        ...
        if(te != null)
        {
            if(te instanceof TileAstralPillarBase)
            {
                success = ((TileAstralPillarBase) te).valid;
            }
        }
