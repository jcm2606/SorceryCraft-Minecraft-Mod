...
import jcm2606.mods.jccore.core.util.ServerUtil;
            ...
            if (args[1].equalsIgnoreCase(Commands.COMMAND_CHARM_CURSE_REMOVE))
            {
                ...
                } else
                {
                    throw new WrongUsageException(
                            "This command has been used incorrectly. This command has to be entered like: /sc curse <command> <command args>");
                }
    }
                    ...
                    } else
                    {
                        ...
                        System.out
                                .println("CHARM @ SLOT " + ServerUtil.getPlayer(commandSender.getCommandSenderName()).inventory.currentItem + " ON PLAYER '" + commandSender
                                        .getCommandSenderName().toUpperCase() + "' HAD CURSE '" + args[2].toUpperCase() + "' APPLIED.");
                    }
                } else
                {
        ...
        if (currentItem != null)
        {
            if (currentItem.getItem() instanceof ItemCharm)
            {
                ...
                if (!ItemCharm.getCurseName(currentItem).equals(""))
                {
                    ...
                    System.out
                            .println("CHARM @ SLOT " + ServerUtil.getPlayer(commandSender.getCommandSenderName()).inventory.currentItem + " ON PLAYER '" + commandSender
                                    .getCommandSenderName().toUpperCase() + "' WAS CLEANSED.");
                } else
                {
