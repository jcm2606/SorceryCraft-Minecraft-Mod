...
public class BlockVordicTorch extends SCBlock
{
    private int count = 0;
    
    public BlockVordicTorch(int par1)
    {
        super(par1, Material.circuits, "vordicTorch");
        ...
        if (par1World.getBlockMetadata(par2, par3, par4) == 0)
        {
            if (par1World.isBlockSolidOnSide(par2 - 1, par3, par4, EAST, true))
            {
                ...
                if (par1World.isBlockSolidOnSide(par2 + 1, par3, par4, WEST, true))
                {
                    ...
                    if (par1World.isBlockSolidOnSide(par2, par3, par4 - 1, SOUTH, true))
                    {
                        par1World.setBlockMetadataWithNotify(par2, par3, par4, 3, 0);
                    } else
                        if (par1World.isBlockSolidOnSide(par2, par3, par4 + 1, NORTH, true))
                        {
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if (var6 == 1 && count == 2)
        {
            SCParticle.spawnVordicFlameFX(var7 - var15, var9 + var13, var11, 0.0D, 0.0D, 0.0D, false);
        } else
            if (var6 == 2 && count == 2)
            {
                SCParticle.spawnVordicFlameFX(var7 + var15, var9 + var13, var11, 0.0D, 0.0D, 0.0D, false);
                ...
                if (var6 == 3 && count == 2)
                {
                    SCParticle.spawnVordicFlameFX(var7, var9 + var13, var11 - var15, 0.0D, 0.0D, 0.0D, false);
                } else
                    if (var6 == 4 && count == 2)
                    {
                        SCParticle.spawnVordicFlameFX(var7, var9 + var13, var11 + var15, 0.0D, 0.0D, 0.0D, false);
                    } else
                    {
                        if (count == 2)
                        {
                            SCParticle.spawnVordicFlameFX(var7, var9, var11, 0.0D, 0.0D, 0.0D, false);
                        }
                    }
