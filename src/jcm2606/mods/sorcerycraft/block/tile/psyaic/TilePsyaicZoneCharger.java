...
public class TilePsyaicZoneCharger extends TileEntityMultiblock implements IEnergyCapacitor, IExpandedSightHandler, IConduitConnector
{
    public int energy;
    ...
    public void readFromNBT(NBTTagCompound tag)
    {
        ...
        this.energy = tag.getInteger("energyStored");
    }
    ...
    public void updateEntity()
    {
        ...
        if (this.isValid)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    this.worldObj.spawnParticle("portal", this.xCoord + 0.5 + (this.worldObj.rand.nextGaussian() / divide),
                            this.yCoord + 0.5 + 1 + (this.worldObj.rand.nextGaussian() / divide), this.zCoord + 0.5 + (this.worldObj.rand
                                    .nextGaussian() / divide), 0, 0, 0);
                    this.worldObj.spawnParticle("portal", this.xCoord + 0.5 - (this.worldObj.rand.nextGaussian() / divide),
                            this.yCoord + 0.5 + 1 - (this.worldObj.rand.nextGaussian() / divide), this.zCoord + 0.5 - (this.worldObj.rand
                                    .nextGaussian() / divide), 0, 0, 0);
                }
            ...
            List playerEntities = this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, AxisAlignedBB.getBoundingBox(this.xCoord - 1.5,
                    this.yCoord + 1, this.zCoord - 1.5, this.xCoord + 1.5, this.yCoord + 3, this.zCoord + 1.5));
            ...
            for (Object obj : playerEntities)
            {
                ...
                if (player.inventory.hasItem(SCObjects.psyonicCellEnergy.itemID))
                {
                    for (int i = 0; i < player.inventory.mainInventory.length; i++)
                    {
                        ...
                        if (stack != null)
                        {
                            if (stack.getItem() instanceof ItemPsyonicEnergyCell)
                            {
                                ...
                                {
                                    cell.setEnergy(stack, cell.getEnergy(stack) + this.takeEnergy(amount));
                                    break;
                                }
                            }
                        }
        ...
        if (player.worldObj.getBlockTileEntity(x, y, z) != null)
        {
            if (player.worldObj.getBlockTileEntity(x, y, z) instanceof IEnergyCapacitor)
            {
                ...
                Minecraft.getMinecraft().fontRenderer.drawString("\2477" + capacitor.getEnergyStored() + " / " + capacitor.getEnergyLimit() + " Psy",
                        RenderUtil.width + 8, RenderUtil.height + 8, 0xffffff);
                GL11.glScaled(1, 1, 1);
                GL11.glPopMatrix();
            }
        }
