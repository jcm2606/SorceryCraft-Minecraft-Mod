...
import jcm2606.mods.sorcerycraft.fx.FXSearFlame;
...
public class ModePyrokenisis extends GauntletMode {
    ...
    public void onGauntletItemUpdateTick(ItemStack stack, EntityPlayer player, World world, int slot, boolean isCurrentItem)
    {
        if(isCurrentItem && world.isRemote && SCApi.astralManager.getTotalEnergyForPlayer(player) > 0)
        {
            ...
            FXSearFlame fx = new FXSearFlame(Minecraft.getMinecraft().theWorld, posX, posY, posZ, (rand.nextFloat() - rand.nextFloat()) * speed, (rand.nextFloat() - rand.nextFloat()) * speed, (rand.nextFloat() - rand.nextFloat()) * speed);
            fx.maxAge = 40;
    ...
    public boolean attemptPlaceFire(EntityPlayer player, World world, int x, int y, int z)
    {
        if(world.getBlockId(x, y + 1, z) == 0)
        {
            ...
            if(world.isRemote)
            {
                for(int i = 0; i < 32; i++)
                {
                    world.spawnParticle("smoke", x + 0.5 - (world.rand.nextGaussian() / 3), y + 1 + 0.5 + (world.rand.nextGaussian() / 3), z + 0.5 - (world.rand.nextGaussian() / 3), 0, 0, 0);
                    world.spawnParticle("smoke", x + 0.5 + (world.rand.nextGaussian() / 3), y + 1 + 0.5 + (world.rand.nextGaussian() / 3), z + 0.5 + (world.rand.nextGaussian() / 3), 0, 0, 0);
                }
                ...
                for(int i = 0; i < 8; i++)
                {
                    if(Minecraft.isFancyGraphicsEnabled() || Settings.RING_MAGMA_FANCY_PARTICLES)
                    {
                        SCParticle.spawnSearFlameFX(x + 0.5 - (world.rand.nextGaussian() / 3), y + 1 + 0.5 + (world.rand.nextGaussian() / 3), z + 0.5 - (world.rand.nextGaussian() / 3), 0, 0.001, 0, false);
                        SCParticle.spawnSearFlameFX(x + 0.5 + (world.rand.nextGaussian() / 3), y + 1 + 0.5 + (world.rand.nextGaussian() / 3), z + 0.5 + (world.rand.nextGaussian() / 3), 0, 0.001, 0, false);
                    } else {
                        world.spawnParticle("flame", x + 0.5 - (world.rand.nextGaussian() / 3), y + 1 + 0.5 + (world.rand.nextGaussian() / 3), z + 0.5 - (world.rand.nextGaussian() / 3), 0, 0, 0);
                        world.spawnParticle("flame", x + 0.5 + (world.rand.nextGaussian() / 3), y + 1 + 0.5 + (world.rand.nextGaussian() / 3), z + 0.5 + (world.rand.nextGaussian() / 3), 0, 0, 0);
                    }
                }
            }
