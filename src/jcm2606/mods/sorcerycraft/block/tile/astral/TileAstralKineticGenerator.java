...
public class TileAstralKineticGenerator extends TileEntityMultiblock implements IEnergyProvider, IExpandedSightHandler
{
    @Override
    ...
    public int provide()
    {
        ...
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS)
        {
            if (GeneralUtil.getBlockTileFromNeighbour(xCoord, yCoord, zCoord, direction, worldObj) != null)
            {
                if (GeneralUtil.getBlockTileFromNeighbour(xCoord, yCoord, zCoord, direction, worldObj) instanceof TileAstralKineticConvertorBase)
                {
                    energy += ((TileAstralKineticConvertorBase) GeneralUtil.getBlockTileFromNeighbour(xCoord, yCoord, zCoord, direction, worldObj))
                            .provide();
                }
            }
        }
    ...
    public void renderOverlay(Minecraft mc, EntityPlayer player, boolean hasMedallion)
    {
        ...
        if (player.worldObj.getBlockTileEntity(x, y, z) != null)
        {
            ...
            Minecraft.getMinecraft().fontRenderer.drawString("\2477+" + this.provide() + " AE/t", RenderUtil.width + 8, RenderUtil.height + 8,
                    0xffffff);
            GL11.glScaled(1, 1, 1);
            GL11.glPopMatrix();
        }
