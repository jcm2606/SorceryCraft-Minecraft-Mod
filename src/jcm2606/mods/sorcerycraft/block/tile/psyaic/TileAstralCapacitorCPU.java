...
public class TileAstralCapacitorCPU extends TileEntityMultiblock implements IEnergyCapacitor, IExpandedSightHandler
{
    ...
    {
        return BlockPsyonicCapacitorCore.capacityIntList[this.getWorldObj().getBlockMetadata(this.xCoord, this.yCoord, this.zCoord)];
    }
    ...
    public boolean isFormedCorrectly()
    {
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS)
        {
            ...
            {
                if (GeneralUtil.getBlockIdFromNeighbour(this.xCoord, this.yCoord, this.zCoord, direction, this.worldObj) != SCObjects.psyonicCapacitorIOInterface.blockID)
                {
                    return false;
                }
            } else
        ...
        for (int x = -1; x < 2; x++)
        {
            for (int y = -1; y < 2; y++)
            {
                for (int z = -1; z < 2; z++)
                {
                    ...
                    if (GeneralUtil.isNeighbouringBlock(new Coord(this.xCoord, this.yCoord, this.zCoord), new Coord(x2, y2, z2), this.worldObj))
                    {
                        continue;
                    }
                    
                    if (this.worldObj.getBlockId(x2, y2, z2) != SCObjects.psyonicCapacitorHousing.blockID)
                    {
                        return false;
                    }
                }
